...
- **检查更新**：设置页「关于」一键检查 GitHub 最新版本，发现新版本可直接下载安装。
- **桌面小组件**：一键添加到桌面，后台定时刷新，显示余额 + 总 Tokens + 最新 UTC+0 日 Tokens。
- **余额预警**：设置阈值，余额低于设定值时给出提醒。
- **液态玻璃 UI**：全新液态玻璃设计 —— 底部导航栏胶囊滑块（阻尼拖拽 / 磁吸档位 / 按压果冻）、全 App 玻璃卡片、多彩环境光斑（或自定义图片）背景下折射呈现。
- **导航栏玻璃效果可调**：设置页可实时调节边缘扭曲（带宽/强度）、背景模糊、玻璃底色、胶囊磨砂、按住外移、边缘模糊、彩虹色散，以及切换粘滞/量化/震动等手感参数。
...
- Kotlin 2.3.21 / AGP 9.4.0 / Gradle 9.6.0
